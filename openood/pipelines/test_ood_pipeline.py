...
from openood.utils import setup_logger, get_config_default
from .temperature import TemperatureWrapper
...
class TestOODPipeline:
    ...
    def run(self):
        ...
        # tune temperature if necessary
        if get_config_default(self.config.pipeline, "tune_temperature", False):
            net = TemperatureWrapper(net)
            net.fit_temperature(id_loader_dict["val"])
